...
def is_ip_host(host: str) -> bool:
    """True, если host — это уже IPv4-адрес, а не домен/сайт."""
    if not host:
        return False
    try:
        import ipaddress
...
class GeoIPResolver:
    ...
    def detect_country(self, config: str) -> tuple[str, str, str] | None:
        ...
        cc, country_name = result
        return _flag(cc), cc, (country_name or cc).lower()
...
def finalize_configs(
    ...
    for cfg in configs:
        ...
        new_remark = f"{flag}{ai_tag} {cc} {country_name}{num_str} | {suffix}"
...
def main():
    ...
    for url in BYPASS_SOURCES:
        ...
        if cfgs:
            cfgs_vless = [cfg for cfg in cfgs if cfg.startswith(ALLOWED_PREFIXES)]
            cfgs_filtered = [cfg for cfg in cfgs_vless if is_ip_host(extract_endpoint_host(cfg))]
            print(f"    ...vless: {len(cfgs_vless)}, из них с IP-хостом: {len(cfgs_filtered)}")
            if cfgs_filtered:
                raw_pools.append(cfgs_filtered)
                all_candidates.extend(cfgs_filtered)

    print(f"  ✓ Всего кандидатов (vless + IP-хост) со всех источников: {len(all_candidates)}")

    geoip = GeoIPResolver()
    geoip.resolve_configs(all_candidates)

    bypass_pools = []
    total_after_preprocess = 0
    for pool in raw_pools:
        valid_p = preprocess_pool(pool, sni_whitelist, geoip)
        total_after_preprocess += len(valid_p)
        if valid_p:
            bypass_pools.append(valid_p)

    print(f"  ✓ Осталось после проверки GeoIP + SNI-whitelist: {total_after_preprocess}")

    print("\n📡 Формирую bypass конфиги (только VLESS, только IP-адреса, 300 штук)...")
    bypass_sampled = sample_from_sources(bypass_pools, 300)
    bypass_final = finalize_configs(bypass_sampled, '@nzea_234', geoip)

    wl_output = '\n'.join([build_header(), '', SEPARATOR_BYPASS, *bypass_final])
    with open('wl_228.txt', 'w', encoding='utf-8') as f:
        f.write(wl_output)

    print(f"\n✅ Готово! wl_228.txt: {len(bypass_final)} (только VLESS, только IP)")
