...
def main():
    ...
    for url in BYPASS_SOURCES:
        ...
        if cfgs:
            ...
            print(f"    ...vless: {len(cfgs_filtered)}")
            if cfgs_filtered:
                raw_pools.append(cfgs_filtered)
                all_candidates.extend(cfgs_filtered)

    print(f"  ✓ Всего кандидатов (vless) со всех источников: {len(all_candidates)}")
    ...
    print("\n📡 Формирую bypass конфиги (только VLESS, 300 штук)...")
    ...
    print(f"\n✅ Готово! wl_228.txt: {len(bypass_final)} (только VLESS)")
